...
volatile int globalShiftLeft=1;
...
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	if(swi_value==0){}
		...
		if((btn_value==swi_value)||(swi_value == 4 && btn_value == 8)){
			if((sign==1) && (countChange_tmr==7)){
				countChange_tmr = 7;
			}

			else if((sign==-1) && (countChange_tmr==0)){countChange_tmr=0;}
			else if(swi_value!=btn_value){}else
...
int main (void)
{
  ...
  while(1){
	  ...
	  if((swi_value>>3)==1){
	  resetState();
	  }else if((swi_value>>2)==1){
		  globalShiftLeft=2;
	  }else if ((swi_value>>1)==1){
		  sign = -1;
	  }
	  else if(swi_value==1){
		  sign = 1;
